...
static bool sendMessage(UserDb *db, int epollfd, user_t *to, user_t *from,
  ...
  send_response_to_user(db, epollfd, to,
                        encodeTextResponseToIMResponseAndFree(rsp, true));
  return true;
}
...
struct IMResponse *cmd_message_impl(UserDb *db, int epollfd,
  ...
  if (req->broadcast) {
    ...
    if (user == NULL) {
      return encodeTextToIMResponse("Recipient user does not exist on server.",
                                    false);
    }
    if (!sendMessage(db, epollfd, user, client->user, (char *)req->msg.value)) {
      return encodeTextToIMResponse(
          "Your message couldn't be delivered because the recipient has "
          "blocked you",
          false);
    }
  }
